...
def main():
    ...
    args.train_url = os.path.join(
        './output/',
        args.train_url,
        f'{args.data}_{args.arch}_eps{args.epochs}_bs{args.batch_size}_lr{args.lr}_lbsmooth0x1_nBlock{args.nBlocks}_stepmode{args.stepmode}_step{args.step}_base{args.base}_nChannels{args.nChannels}_mhidden{args.meta_net_hidden_size}_mlayers{args.meta_net_num_layers}_mintv{args.meta_interval}_mlr{args.meta_lr}_mwd{args.meta_weight_decay}_meta_inp_type_{args.meta_net_input_type}_constrdim_{args.constraint_dimension}_epsilon{args.epsilon:.2f}_targetp{args.target_p_index:02d}/',
        )
...
def load_imagenet_data(args):
    """ ImageFolder Style """
    """traindir = os.path.join(f'{args.data_url}', 'imagenet', 'train')
    valdir = os.path.join(f'{args.data_url}', 'imagenet', 'val')
