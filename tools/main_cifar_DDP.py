...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # ImageNet Dataloader
    train_loader, _, test_loader, train_sampler = load_cifar_data(args)
...
def load_cifar_data(args):
    ...
    if args.data == 'cifar10':
        ...
        val_set = datasets.CIFAR10(args.data_url, train=False,download=True,
                                   transform=transforms.Compose([
                                    transforms.ToTensor(),
                                    normalize
                                   ]))
        ...
        val_set = datasets.CIFAR100(args.data_url, train=False,download=True,
                                    transform=transforms.Compose([
                                        transforms.ToTensor(),
                                        normalize
                                    ]))
    ...
    if 'train' in args.splits:
        train_loader = torch.utils.data.DataLoader(
            ...
            sampler=train_sampler, shuffle=(train_sampler==None),
            num_workers=args.workers, pin_memory=False)
    if 'val' or 'test' in args.splits:
        val_loader = torch.utils.data.DataLoader(
            val_set,
            batch_size=args.batch_size,
            sampler=test_sampler, shuffle=(test_sampler==None),
            num_workers=args.workers, pin_memory=False)
